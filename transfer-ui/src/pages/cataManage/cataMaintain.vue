...
<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="操作" align="center" width="200" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          ...
          <el-button v-if="row.ifShowStop === 'Y'" type="primary" size="mini" plain @click="stopUsing(row.catalogId)">
            停用
          </el-button>
          <!-- TODO 多个提交 -->
          <!-- <el-button v-if="row.ifShowSub === 'Y'" type="primary" size="mini" plain @click="catalogSubmit(row.catalogId,'2')">
            提交
          </el-button> -->
        </template>
      </el-table-column>
    ...
    <el-dialog :visible.sync="stopVisible" :title="titleMap[dialogStatus]" :close-on-click-modal="false" width="60%">
      <el-form ref="stopForm" :model="stopTemp" label-position="top" class="form-main" :rules="stopFormRules">
        <el-form-item label="停用生效日期：" prop="catalogStopType">
          <el-radio-group v-model="stopTemp.catalogStopType">
            <el-radio label="1">指定失效日期</el-radio>
            <el-radio label="2">立即失效</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="stopTemp.catalogStopType == 1" prop="catalogStopEffectiveDate">
          <el-date-picker
            v-model="stopTemp.catalogStopEffectiveDate"
            type="date"
            placeholder="请填写失效日期"
            value-format="yyyy-MM-dd"
          />
        </el-form-item>
        <el-form-item label="关联证照的处理办法：" prop="relatedCertificateStopType">
          <el-radio-group v-model="stopTemp.relatedCertificateStopType">
            <el-radio label="0">保持有效</el-radio>
            <el-radio label="1">立即失效</el-radio>
            <el-radio label="2">指定日期失效</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item v-if="stopTemp.relatedCertificateStopType == 2" prop="relatedCertificateStopDate">
          <el-date-picker
            v-model="stopTemp.relatedCertificateStopDate"
            type="date"
            placeholder="请填写失效日期"
            value-format="yyyy-MM-dd"
          />
        ...
        <el-form-item label="备注：">
          <el-input v-model="stopTemp.remarks" type="textarea" />
        </el-form-item>
      </el-form>
      <div slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="submitForm('stopForm')">
          确定
        </el-button>
      </div>
    </el-dialog>
...
import { getCatalogPageData, queryDictItemsByTypeCode, auditCatalog, catalogSub } from '@/api/licenseCata'
import { catalogStopUsing } from '@/api/cataMaintain'
...
export default {
  ...
  data() {
    return {
      ...
      stopTemp: {
        catalogId: '',
        catalogStopType: '',
        catalogStopEffectiveDate: '',
        relatedCertificateStopDate: '',
        relatedCertificateStopType: '',
        remarks: ''
      },
      stopFormRules: {
        catalogStopType: [
          { required: true, message: '请选择停用生效日期类型', trigger: 'change' }
        ],
        relatedCertificateStopType: [
          { required: true, message: '请选择关联证照的处理办法', trigger: 'change' }
        ],
        catalogStopEffectiveDate: [
  ...
  methods: {
    ...
    stopUsing(catalogId) {
      ...
      this.dialogStatus = 'stop'
      this.stopTemp.catalogId = catalogId
      this.$nextTick(() => {
        this.$refs['stopForm'].clearValidate()
        this.$refs['stopForm'].resetFields()
        this.stopTemp.remarks = ''
      })
    },
    // 提交
    // 提交目录
    catalogSubmit(catalogId, catalogStatus) {
      catalogSub({ catalogId: catalogId, catalogStatus: catalogStatus }).then(response => {
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          catalogStopUsing(this.stopTemp).then(response => {
            if (response.data.data.status === true) {
              this.$message({
